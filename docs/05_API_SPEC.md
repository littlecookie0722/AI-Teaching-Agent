...
- `/api/review-task-summary` 只读本地 Mock 审核队列摘要，可按 `status`、`taskType`、`limit`、`detailMode` 查询。追加 `workflowRunId=<id>` 时只返回该运行关联的任务；运行不存在返回统一 `NOT_FOUND`。对于 `artifactProfile=teaching-core` 的 Phase 2 运行，响应增加 `reviewTaskSummary.teachingPackageReview`（`TeachingPackageReviewSummary`），按 Lab / Exam / Grading 当前任务状态派生 `WAITING_REVIEW`、`NEEDS_REVISION` 或 `APPROVED`，并返回三类 Artifact 路径、Schema/内容质量摘要、候选人安全摘要、审核进度、逐任务 approve/reject 契约和 `exportReady`。该摘要本身只读；人工决定仍逐项调用既有 `POST /api/ai-tasks/{id}/approve` 或 `/reject`，reject 必须提供 reason，批量 approve/reject/publish 固定禁用。未提供 `workflowRunId` 时继续返回原有 `reviewPriorityQueue` 和 `realDemoReviewQueue`；历史 `legacy-all` 运行可按运行过滤原有四类任务，但 `teachingPackageReview.available=false`。
