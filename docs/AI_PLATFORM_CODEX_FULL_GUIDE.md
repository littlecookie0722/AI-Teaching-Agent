...
> 文档角色：本文件是架构、DSL、API、Phase 任务和历史演进的完整参考，不是当前逐回合规则源。当前执行约束以项目根目录 `AGENTS.md` 为准，当前路线和停止线以 `docs/24_PROJECT_PROGRESS_MAP.md` 为准；发生冲突时不按本文件中的历史“下一步”执行。
>
> 使用方式：需要架构背景或模块细节时按需查阅本文件，不要求每轮全文重读。第 3 章、第 16-18 章和第 22 章包含历史 Codex 工作方式、提示词和阶段切换记录，不能覆盖当前 `AGENTS.md`。
...
> 本章保留早期工作基线。当前开发前/开发后流程、输出格式和测试适用范围以 `AGENTS.md` 第 6-7 节为准。
...
# 22. 阶段封口与核心业务切换规则（历史归档）

> 本章记录封口规则的演进背景。当前封口、核心范围和路线停止线以 `AGENTS.md` 第 3 节及 `docs/24_PROJECT_PROGRESS_MAP.md` 为准；本章不再作为单独规则源。
