...
- 标题、副标题和各版式 bullet 使用与画布一致的安全长度；跨平台字体下 hero / exercise / summary 标题上限分别为 22 / 23 / 21 字符，其他标题上限为 24。超限兼容 DSL 会返回 `SLIDE_TITLE_LONG`、`SUBTITLE_TEXT_LONG` 或 `BULLET_TEXT_LONG`，PPTX 与 PNG 使用相同带省略号的显示文本。
