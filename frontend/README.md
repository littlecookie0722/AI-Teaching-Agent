...
> 当前产品主线只保留 `generation-workspace.html` 和 `review-center.html`：从一份 Markdown 生成 Lab + Exam/Grading，完成逐项人工审核，并在全部批准后导出本地教学包。其他页面保留为兼容、诊断或历史 PoC，不作为当前主导航。入口决策和待实现差距见 `docs/28_SIMPLIFIED_MVP_ENTRYPOINTS.md`。
...
- `review-center.html`：唯一默认审核与导出入口。通过 `workflowRunId` 聚合 Lab / Exam / Grading 的任务状态、Schema/质量信号、候选人安全和导出就绪状态，在同一流程中复用已有逐任务 approve/reject 动作，并仅在服务端返回 `exportReady=true` 时调用本地教学包导出；不提供批量决定或发布。
...
默认审核深链为 `review-center.html?taskId=<taskId>&workflowRunId=<workflowRunId>`。页面调用 `GET /api/review-task-summary?detailMode=light&workflowRunId=...`，在顶部固定呈现 Lab、Exam、Grading 三行及审核进度、Schema 通过数、候选人安全和 `exportReady`；教学包激活时不混入历史 PPT/real-demo 队列卡片。每行“通过 / 退回”仍调用既有 `POST /api/ai-tasks/{id}/approve|reject`，要求 reviewer，退回额外要求 reason，并在动作后回读同一批次状态。三项全部批准且服务端摘要明确 `exportReady=true` 后，页面才启用唯一“导出教学包”按钮并调用 `POST /api/teaching-packages/export`；请求只携带 `workflowRunId` 和 reviewer，响应只展示 `outputPath`、文件名、摘要哈希、大小和文件数量等安全元数据。服务端仍会再次校验审核与候选人安全状态；页面不使用 Blob、不在客户端拼包或读取 DSL 文件。Exam 行及导出结果都不渲染答案或内部 `gradingRef`；页面不批量审核、不自动发布、不调用真实 LLM。
...
- `generation-workspace.html`: 本地核心一键生成工作台。页面通过单一 `POST /api/phase2/workflows/content-generation/run` 和 `artifactProfile=teaching-core` 从一份本地教学素材生成 Lab / Exam / Grading 三类 DSL，展示批次进度、Schema/内容质量摘要、三个任务 ID 和对应审核入口；所有任务固定进入 `WAITING_REVIEW`。默认使用 Mock Provider；真实模式必须显式确认，模型密钥只由同源后端从环境变量读取。页面不直接调用模型 Provider、不接收 API Key、不自动审核、不发布、不执行选手代码，并固定 `answerVisibleToCandidate=false`、`gradingRefVisibleToCandidate=false`、`autoPublishAllowed=false`。
