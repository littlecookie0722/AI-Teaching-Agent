...
from backend.core_mysql_migration import run_mysql_backend_core_smoke
from backend.core_service import BACKEND_CORE_DATABASE_URL_ENV


@pytest.mark.integration
def test_backend_core_mysql_real_smoke(tmp_path):
    if os.environ.get("LAB_BACKEND_CORE_MYSQL_SMOKE") != "1":
        pytest.skip("set LAB_BACKEND_CORE_MYSQL_SMOKE=1 to run real MySQL smoke")
    if not os.environ.get(BACKEND_CORE_DATABASE_URL_ENV):
        pytest.skip(f"set {BACKEND_CORE_DATABASE_URL_ENV} to a test/staging MySQL URL")

    result = run_mysql_backend_core_smoke(
        tmp_path,
        reviewer=os.environ.get("LAB_BACKEND_CORE_MYSQL_SMOKE_REVIEWER", "mysql_smoke_ci"),
    )

    assert result["mode"] == "BACKEND_CORE_MYSQL_SMOKE"
