...
def test_postgresql_migration_plan_is_redacted_and_does_not_connect(monkeypatch, tmp_path):
    monkeypatch.setenv(BACKEND_CORE_DATABASE_URL_ENV, "postgresql://user:secret@example.invalid/prod")

    plan = build_postgresql_migration_plan(tmp_path)
    ...
    assert plan["productionDatabaseWritten"] is False
    assert "user:secret" not in json.dumps(plan, ensure_ascii=False)


def test_postgresql_migration_init_and_summary_use_connector(monkeypatch, tmp_path):
    monkeypatch.setenv(BACKEND_CORE_DATABASE_URL_ENV, "postgresql://user:secret@example.invalid/prod")
    database = FakePostgreSQLDatabase()

    init_result = initialize_postgresql_backend_core_repository(
        tmp_path,
        connector=database.connect,
    )
    summary_result = summarize_postgresql_backend_core_repository(
        tmp_path,
        connector=database.connect,
    )

    assert init_result["mode"] == "BACKEND_CORE_POSTGRESQL_SCHEMA_INIT"
    ...
    assert summary_result["mode"] == "BACKEND_CORE_POSTGRESQL_SCHEMA_SUMMARY"
...
def test_postgresql_smoke_round_trips_task_artifact_and_audit(monkeypatch, tmp_path):
    monkeypatch.setenv(BACKEND_CORE_DATABASE_URL_ENV, "postgresql://user:secret@example.invalid/prod")
    database = FakePostgreSQLDatabase()

    smoke = run_postgresql_backend_core_smoke(
        tmp_path,
...
def test_postgresql_migration_rejects_missing_database_url(monkeypatch, tmp_path):
    monkeypatch.delenv(BACKEND_CORE_DATABASE_URL_ENV, raising=False)

    with pytest.raises(CoreRepositoryError) as exc_info:
        build_postgresql_migration_plan(tmp_path)

    assert exc_info.value.code == "BACKEND_CORE_POSTGRESQL_DATABASE_URL_MISSING"
