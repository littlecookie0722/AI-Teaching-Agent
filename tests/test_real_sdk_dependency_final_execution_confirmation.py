...
def assert_safe_final_confirmation_context(context):
    for key in [
        ...
        "secretValueRead",
        "networkAccess",
        "realCallAfterFinalConfirmationAuthorized",
        "realLlmCalled",
...
def confirmed_request(**overrides):
    payload = {
        "provider_id": "openai",
        "approval_ref": "FINAL-CONFIRM-001",
        "reviewer": "teacher_1",
    ...
    return RealSdkDependencyFinalExecutionConfirmationRequest(**payload)
...
def test_real_sdk_dependency_final_execution_confirmation_contract_is_mock_only_and_local():
    contract = load_json("providers/real-sdk-dependency-final-execution-confirmation.contract.json")
    ...
    assert contract["gateId"] == "real_sdk_dependency_final_execution_confirmation"
    assert contract["activeProvider"] == "mock"
    assert contract["supportedProvider"] == "openai"
    assert contract["requiredContext"]["finalExecutionConfirmationOnly"] is True
    assert contract["requiredContext"]["finalExecutionConfirmationReady"] is False
    assert contract["requiredContext"]["executionApprovalGranted"] is False
    assert contract["requiredContext"]["executionTaskCreated"] is False
    assert contract["requiredContext"]["dependencyFileMutationAuthorized"] is False
    assert "grant_execution_approval" in contract["blockedOperations"]
    assert "create_dependency_execution_task" in contract["blockedOperations"]
    ...
    assert "test_real_sdk_dependency_final_execution_confirmation" in contract["recommendedCommandIds"]
...
def test_real_sdk_dependency_final_execution_confirmation_describe_is_disabled_and_safe():
    descriptor = describe_real_sdk_dependency_final_execution_confirmation(root=ROOT)

    assert descriptor["finalExecutionConfirmationId"] == "real_sdk_dependency_final_execution_confirmation"
    assert descriptor["gateMode"] == "DEPENDENCY_FINAL_EXECUTION_CONFIRMATION_DISABLED_ONLY"
    assert descriptor["defaultProvider"] == "mock"
    assert descriptor["requiresReadonlyDiffReviewReady"] is True
    assert descriptor["finalExecutionConfirmationOnly"] is True
    assert "future_reviewed_dependency_file_change_task" in descriptor["pipeline"]
    assert_safe_final_confirmation_context(descriptor)


def test_real_sdk_dependency_final_execution_confirmation_default_is_not_ready_and_redacts_payload(monkeypatch):
    fake_key = "sk-" + "final-confirm-hidden"
    monkeypatch.setenv("OPENAI_API_KEY", fake_key)
    result = build_real_sdk_dependency_final_execution_confirmation(
        RealSdkDependencyFinalExecutionConfirmationRequest(
            provider_id="openai",
    ...
    checklist = {item["id"]: item for item in result["finalExecutionConfirmationChecklist"]}
    assert result["readonlyDiffReviewReady"] is False
    assert result["finalExecutionConfirmationReady"] is False
    assert result["readyForReviewedDependencyExecutionTask"] is False
    assert checklist["readonly_diff_review_ready"]["passed"] is False
    assert checklist["final_approver_identity_confirmed"]["passed"] is False
    assert fake_key not in serialized
    assert "[REDACTED]" in serialized
    assert_safe_final_confirmation_context(result)


def test_real_sdk_dependency_final_execution_confirmation_readonly_ready_but_missing_final_confirmations():
    request = confirmed_request(
        readonly_diff_review_confirmed=False,
        final_approver_identity_confirmed=False,
    )
    ...
    assert result["readonlyDiffReviewSummary"]["readyForReadonlyDependencyDiffReview"] is True
    assert result["finalExecutionConfirmationReady"] is False
    assert result["readyForReviewedDependencyExecutionTask"] is False
    assert checklist["readonly_diff_review_ready"]["passed"] is True
    assert checklist["readonly_diff_review_confirmed"]["passed"] is False
    assert checklist["final_approver_identity_confirmed"]["passed"] is False
    assert_safe_final_confirmation_context(result)


def test_real_sdk_dependency_final_execution_confirmation_ready_but_no_execution_or_file_mutation():
    result = build_real_sdk_dependency_final_execution_confirmation(confirmed_request(), root=ROOT)
    checklist = {item["id"]: item for item in result["finalExecutionConfirmationChecklist"]}

    assert result["readonlyDiffReviewReady"] is True
    assert result["finalExecutionConfirmationReady"] is True
    assert result["readyForReviewedDependencyExecutionTask"] is True
    assert result["executionApprovalGranted"] is False
    assert result["executionTaskCreated"] is False
    assert result["dependencyFileMutationAuthorized"] is False
    assert checklist["no_execution_authorization_confirmed"]["passed"] is True
    assert checklist["no_dependency_file_mutation_confirmed"]["passed"] is True
    assert result["finalConfirmationModel"]["materializedNow"] is False
    assert result["finalConfirmationModel"]["writeNow"] is False
    assert all(item["mutateNow"] is False for item in result["finalConfirmationModel"]["targetFiles"])
    assert all(item["writeNow"] is False for item in result["finalConfirmationModel"]["targetFiles"])
    assert all(item["materializedNow"] is False for item in result["finalConfirmationModel"]["requiredEvidence"])
    assert result["futureChangeEnvelope"]["executionApprovalGranted"] is False
    assert result["futureChangeEnvelope"]["executionTaskCreated"] is False
    assert result["futureChangeEnvelope"]["dependencyFileMutationAuthorized"] is False
    assert result["futureChangeEnvelope"]["commandExecuted"] is False
    assert result["futureChangeEnvelope"]["realLlmCalled"] is False
    assert_safe_final_confirmation_context(result)


def test_real_sdk_dependency_final_execution_confirmation_invalid_scope_keeps_safe_error_context():
    request = confirmed_request(output_kind="Exam")

    try:
        build_real_sdk_dependency_final_execution_confirmation(request, root=ROOT)
    except ProviderError as exc:
        assert exc.code == "VALIDATION_ERROR"
        context = build_real_sdk_dependency_final_execution_confirmation_error_context(
            exc,
            request=request,
            root=ROOT,
        )
    ...
    assert context["finalExecutionConfirmationReady"] is False
    assert context["outputKind"] == "Exam"
    assert_safe_final_confirmation_context(context)


def test_real_sdk_dependency_final_execution_confirmation_cli_describe_and_confirm_paths(capsys):
    exit_code, payload = run_cli(
        ["provider", "real-sdk-dependency-final-execution-confirmation", "describe"],
        capsys,
    )
    ...
    assert payload["data"]["finalExecutionConfirmationId"] == "real_sdk_dependency_final_execution_confirmation"
    assert payload["data"]["finalExecutionConfirmationReady"] is False
    assert_safe_final_confirmation_context(payload["data"])

    exit_code, payload = run_cli(
        ["provider", "real-sdk-dependency-final-execution-confirmation", "confirm", "--provider", "openai"],
        capsys,
    )
    ...
    assert payload["data"]["readyForReviewedDependencyExecutionTask"] is True
    assert payload["data"]["executionApprovalGranted"] is False
    assert payload["data"]["dependencyFileChanged"] is False
    assert_safe_final_confirmation_context(payload["data"])


def test_real_sdk_dependency_final_execution_confirmation_cli_rejects_bad_payload_and_invalid_scope(capsys):
    exit_code, payload = run_cli(
        [
            "provider",
            "real-sdk-dependency-final-execution-confirmation",
    ...
    assert_json_envelope(payload)
    assert payload["code"] == "VALIDATION_ERROR"
    context = payload["realSdkDependencyFinalExecutionConfirmationContext"]
    assert context["outputKind"] == "Exam"
    assert_safe_final_confirmation_context(context)
