...
def confirmed_request(**overrides):
    payload = {
        ...
        "reviewer": "teacher_1",
        "approval_ref": "MINIMAL-CALL-POC-001",
    ...
    return RealLlmMinimalCallPocReviewRequest(**payload)
...
def test_contract_declares_minimal_call_review_only():
    contract = load_json("providers/real-llm-minimal-call-poc-review.contract.json")

    assert contract["phase"] == "Phase 2"
    assert contract["mode"] == "REAL_LLM_MINIMAL_CALL_POC_REVIEW_ONLY"
    assert contract["upstreamGateId"] == "real_llm_pre_send_dry_run_record"
    assert contract["rules"]["requiresPreSendDryRunRecordReady"] is True
    assert contract["rules"]["realCallAuthorized"] is False
    assert contract["assertions"]["minimalCallPocReviewReady"] is True
    ...
    assert contract["futureSendBoundary"]["sendAllowedInThisReview"] is False
    assert contract["futureSendBoundary"]["singleRequestOnly"] is True


def test_describe_is_safe_and_does_not_authorize_send():
    descriptor = describe_real_llm_minimal_call_poc_review(root=ROOT)

    assert descriptor["minimalCallPocReviewId"] == "real_llm_minimal_call_poc_review"
    assert descriptor["reviewMode"] == "MINIMAL_REAL_CALL_IMPLEMENTATION_REVIEW_ONLY"
...
def test_review_requires_upstream_pre_send_record_before_build():
    with pytest.raises(ProviderError) as exc_info:
        build_real_llm_minimal_call_poc_review(
            RealLlmMinimalCallPocReviewRequest(provider_id="openai"),
            root=ROOT,
        )

    exc = exc_info.value
    assert exc.code == "REAL_LLM_REQUEST_REVIEW_CONFIRMATION_REQUIRED"
    context = build_real_llm_minimal_call_poc_review_error_context(
        exc,
        request=RealLlmMinimalCallPocReviewRequest(provider_id="openai"),
        root=ROOT,
    )
...
def test_review_ready_still_does_not_send_or_read_secret():
    secret = "fake-secret-that-must-be-redacted"
    result = build_real_llm_minimal_call_poc_review(confirmed_request(), root=ROOT)
    ...
    assert result["explicitPreSendDryRunRecordOptIn"] is True
    assert result["readyForMinimalRealCallPocReview"] is True
    assert result["dryRunRecordBuilt"] is True
    assert result["minimalCallPocChecklistReady"] is True
    assert result["minimalCallPocReviewReady"] is True
    assert result["readyForMinimalRealCallImplementation"] is True
    assert result["readyForRealRequestSend"] is False
    assert result["requestSendBoundary"]["singleRequestOnly"] is True
    assert result["requestSendBoundary"]["labOnly"] is True
    assert result["requestSendBoundary"]["sendAllowedInThisReview"] is False
    assert result["secretPolicy"]["readAllowedInThisReview"] is False
    assert result["secretPolicy"]["logSecretValue"] is False
    assert result["responseValidationPolicy"]["validResponseDefaultStatus"] == "WAITING_REVIEW"
    assert result["failureRollbackPolicy"]["safeFailureLeavesGeneratedContent"] is False
    assert result["implementationReviewPackage"]["built"] is True
    assert result["implementationReviewPackage"]["requestSendAllowedNow"] is False
...
def test_review_with_missing_poc_flags_is_not_ready():
    result = build_real_llm_minimal_call_poc_review(
        confirmed_request(
            single_request_scope_confirmed=False,
            env_secret_source_confirmed=False,
            no_real_request_send_in_review_confirmed=False,
        ),
        root=ROOT,
    )
    checklist = {item["id"]: item for item in result["minimalCallPocChecklist"]}

    assert result["preSendDryRunRecordReady"] is True
    assert result["minimalCallPocReviewReady"] is False
    assert result["readyForMinimalRealCallImplementation"] is False
    assert checklist["pre_send_dry_run_record_ready"]["passed"] is True
    assert checklist["single_request_scope_confirmed"]["passed"] is False
    assert checklist["env_secret_source_confirmed"]["passed"] is False
    assert checklist["no_real_request_send_in_review_confirmed"]["passed"] is False
    assert_no_real_call(result)
...
def test_invalid_provider_is_rejected_safely():
    ...
    with pytest.raises(ProviderError) as exc_info:
        build_real_llm_minimal_call_poc_review(request, root=ROOT)

    assert exc_info.value.code == "VALIDATION_ERROR"
    context = build_real_llm_minimal_call_poc_review_error_context(exc_info.value, request=request, root=ROOT)
...
def test_cli_describe_and_review_return_json(capsys):
    exit_code, payload = run_cli(["provider", "real-llm-minimal-call-poc-review", "describe"], capsys)

    assert exit_code == 0
    assert payload["success"] is True
    assert payload["code"] == "OK"
    assert payload["data"]["minimalCallPocReviewReady"] is False
    ...
    assert payload["data"]["readyForMinimalRealCallImplementation"] is True
...
def test_cli_review_missing_upstream_confirmations_returns_safe_error(capsys):
    exit_code, payload = run_cli(
        ["provider", "real-llm-minimal-call-poc-review", "review", "--provider", "openai"],
        capsys,
    )
    ...
    context = payload["realLlmMinimalCallPocReviewContext"]
    assert context["preSendDryRunRecordReady"] is False
    assert context["minimalCallPocReviewReady"] is False
    assert_no_real_call(context)
