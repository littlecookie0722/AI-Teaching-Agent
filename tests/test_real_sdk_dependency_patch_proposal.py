...
def assert_safe_patch_proposal_context(context):
    for key in [
...
def confirmed_request(**overrides):
    ...
    return RealSdkDependencyPatchProposalRequest(**payload)


def confirmed_cli_args():
    return [
        "provider",
        "real-sdk-dependency-patch-proposal",
        "propose",
        "--provider",
        "openai",
        "--approval-ref",
...
def test_real_sdk_dependency_patch_proposal_contract_is_mock_only_and_local():
    contract = load_json("providers/real-sdk-dependency-patch-proposal.contract.json")
    ...
    assert contract["proposalId"] == "real_sdk_dependency_patch_proposal"
    assert contract["activeProvider"] == "mock"
    assert contract["supportedProvider"] == "openai"
    assert contract["requiredContext"]["changePreviewRequired"] is True
    assert contract["requiredContext"]["changePreviewReady"] is False
    assert contract["requiredContext"]["dependencyPatchProposalOnly"] is True
    assert contract["requiredContext"]["patchFileWritten"] is False
    ...
    assert "test_real_sdk_dependency_patch_proposal" in contract["recommendedCommandIds"]
...
def test_real_sdk_dependency_patch_proposal_describe_is_disabled_and_safe():
    descriptor = describe_real_sdk_dependency_patch_proposal(root=ROOT)

    assert descriptor["proposalId"] == "real_sdk_dependency_patch_proposal"
    assert descriptor["proposalMode"] == "DEPENDENCY_PATCH_PROPOSAL_DISABLED_ONLY"
    assert descriptor["defaultProvider"] == "mock"
    assert descriptor["requiresChangePreview"] is True
    assert descriptor["dependencyPatchProposalOnly"] is True
    assert "future_dependency_patch_implementation_task" in descriptor["pipeline"]
    assert_safe_patch_proposal_context(descriptor)


def test_real_sdk_dependency_patch_proposal_default_is_not_ready_and_redacts_payload(monkeypatch):
    fake_key = "sk-" + "patch-proposal-hidden"
    monkeypatch.setenv("OPENAI_API_KEY", fake_key)
    result = build_real_sdk_dependency_patch_proposal(
        RealSdkDependencyPatchProposalRequest(
            provider_id="openai",
    ...
    checklist = {item["id"]: item for item in result["patchProposalChecklist"]}
    assert result["changePreviewReady"] is False
    assert result["patchProposalChecklistPassed"] is False
    assert result["readyForDependencyPatchImplementationTask"] is False
    assert checklist["change_preview_ready"]["passed"] is False
    assert checklist["patch_plan_review_confirmed"]["passed"] is False
    assert fake_key not in serialized
    assert "[REDACTED]" in serialized
    assert_safe_patch_proposal_context(result)


def test_real_sdk_dependency_patch_proposal_preview_ready_but_missing_patch_confirmations():
    request = confirmed_request(patch_scope_confirmed=False, patch_plan_review_confirmed=False)
    result = build_real_sdk_dependency_patch_proposal(request, root=ROOT)
    checklist = {item["id"]: item for item in result["patchProposalChecklist"]}

    assert result["changePreviewReady"] is True
    assert result["changePreviewSummary"]["readyForDependencyChangeImplementationTask"] is True
    assert result["patchProposalChecklistPassed"] is False
    assert result["readyForDependencyPatchImplementationTask"] is False
    assert checklist["change_preview_ready"]["passed"] is True
    assert checklist["patch_scope_confirmed"]["passed"] is False
    assert checklist["patch_plan_review_confirmed"]["passed"] is False
    assert_safe_patch_proposal_context(result)


def test_real_sdk_dependency_patch_proposal_all_confirmations_ready_but_still_no_patch_or_apply():
    result = build_real_sdk_dependency_patch_proposal(confirmed_request(), root=ROOT)
    checklist = {item["id"]: item for item in result["patchProposalChecklist"]}

    assert result["changePreviewReady"] is True
    assert result["patchProposalChecklistPassed"] is True
    assert result["readyForDependencyPatchImplementationTask"] is True
    assert result["dependencyPatchProposalReady"] is True
    assert checklist["no_patch_apply_confirmed"]["passed"] is True
    assert all(item["writeNow"] is False for item in result["patchPlan"])
    assert all(item["applyNow"] is False for item in result["patchPlan"])
    assert result["patchApplyPolicy"]["patchFileWritten"] is False
    assert result["patchApplyPolicy"]["patchApplied"] is False
    assert result["patchApplyPolicy"]["diffArtifactWritten"] is False
    assert_safe_patch_proposal_context(result)


def test_real_sdk_dependency_patch_proposal_invalid_scope_keeps_safe_error_context():
    request = confirmed_request(output_kind="Exam")

    try:
        build_real_sdk_dependency_patch_proposal(request, root=ROOT)
    except ProviderError as exc:
        assert exc.code == "VALIDATION_ERROR"
        context = build_real_sdk_dependency_patch_proposal_error_context(exc, request=request, root=ROOT)
    else:
        raise AssertionError("expected ProviderError")

    assert context["errorCode"] == "VALIDATION_ERROR"
    assert context["changePreviewReady"] is False
    assert context["outputKind"] == "Exam"
    assert_safe_patch_proposal_context(context)


def test_real_sdk_dependency_patch_proposal_cli_describe_and_propose_paths(capsys):
    exit_code, payload = run_cli(["provider", "real-sdk-dependency-patch-proposal", "describe"], capsys)

    assert exit_code == 0
    assert_json_envelope(payload)
    assert payload["success"] is True
    assert payload["data"]["proposalId"] == "real_sdk_dependency_patch_proposal"
    assert payload["data"]["readyForDependencyPatchImplementationTask"] is False
    assert_safe_patch_proposal_context(payload["data"])

    exit_code, payload = run_cli(
        ["provider", "real-sdk-dependency-patch-proposal", "propose", "--provider", "openai"],
        capsys,
    )
    ...
    assert_json_envelope(payload)
    assert payload["success"] is True
    assert payload["data"]["changePreviewReady"] is False
    assert payload["data"]["patchProposalChecklistPassed"] is False
    assert_safe_patch_proposal_context(payload["data"])
    ...
    assert_json_envelope(payload)
    assert payload["success"] is True
    assert payload["data"]["changePreviewReady"] is True
    assert payload["data"]["patchProposalChecklistPassed"] is True
    assert payload["data"]["readyForDependencyPatchImplementationTask"] is True
    assert_safe_patch_proposal_context(payload["data"])


def test_real_sdk_dependency_patch_proposal_cli_rejects_bad_payload_and_invalid_scope(capsys):
    exit_code, payload = run_cli(
        ["provider", "real-sdk-dependency-patch-proposal", "propose", "--provider", "openai", "--payload", "not-json"],
        capsys,
    )
    ...
    exit_code, payload = run_cli(
        ["provider", "real-sdk-dependency-patch-proposal", "propose", "--provider", "openai", "--output-kind", "Exam"],
        capsys,
    )
    ...
    assert_json_envelope(payload)
    assert payload["code"] == "VALIDATION_ERROR"
    context = payload["realSdkDependencyPatchProposalContext"]
    assert context["outputKind"] == "Exam"
    assert_safe_patch_proposal_context(context)
